...
// this file implements the current placeholder b*+-tree behavior on top of
// std::map.
#include <algorithm>
...
namespace dbms::index {
    ...
    std::vector<common::RowId>
    BStarPlusTree::Find(const std::string &key) const {
        auto it = entries_.find(key);
        if (it == entries_.end()) {
            return {};
        }
    ...
    BStarPlusTree::RangeScan(const std::string &lower_bound,
        ...
        for (auto it = entries_.lower_bound(lower_bound);
             it != entries_.end() && it->first < upper_bound; ++it) {
