    ...
    TableRuntime CloneTableRuntime(const TableRuntime &source) {
        ...
        for (const auto &row : rows) {
            for (const auto &[column_name, index_tree] : cloned.indexes) {
                ...
                if (std::holds_alternative<std::monostate>(value)) {
                    index_tree->Insert("N", row.row_id);
                    ...
                    index_tree->Insert(output.str(), row.row_id);
                } else {
                    index_tree->Insert("S" + common::AsString(value), row.row_id);
                }
            }
        }
        return cloned;
            ...
            for (const auto &[table_name, table_runtime] : database_runtime.tables) {
                ...
                for (const auto &[indexed_column_name, index_tree] :
                    ...
                    for (const auto &row : rows) {
                        ...
                        if (found.size() != 1 || found[0] != row.row_id) {
                            return common::MakeError<bool>(
                                common::ErrorCode::kStorageError,
                    ...
                    for (auto &[indexed_column_name, index_tree] :
                        ...
                        const auto encoded =
                            EncodeIndexKey(row.values[*column_index],
                                           table_runtime.schema.columns[*column_index]
                                               .type);
                        index_tree->Insert(encoded, row.row_id);
                    }
                }
            }
        }
